...
cdef extern from "chealpix.h": 
  ...
  long _xyf2ring "xyf2ring" (long _nside, int _ix, int _iy, int _face_num) nogil
...
def ang2ngb_ring (nside,theta,phi, ipixvec = None,wvec = None):
  "ang2ngb_ring"
  shape = numpy.broadcast(1, nside,theta,phi).shape 
  if ipixvec is None: ipixvec = numpy.empty(shape, dtype=('int', 5))
  ...
  iter = numpy.nditer([nside,theta,phi,ipixvec[...,0],ipixvec[...,1],ipixvec[...,2],ipixvec[...,3],ipixvec[...,4],wvec[...,0],wvec[...,1],wvec[...,2],wvec[...,3],wvec[...,4]],
       op_dtypes=['int','f8','f8','int','int','int','int','int','f8','f8','f8','f8','f8'],
  ...
  cdef double _theta
  cdef double _phi
  cdef numpy.ndarray ipixvec_a = numpy.empty(0, dtype=('int', 5))
  ...
  with nogil:
    while size >0:
      while size > 0:
        ...
        (<long *> citer.data[6])[0] = _ipixvec[3] 
        (<long *> citer.data[7])[0] = _ipixvec[4] 
  ...
  with nogil:
    while size >0:
      while size > 0:
        _nside = (<long * > citer.data[0])[0] 
        _ipix = (<long * > citer.data[1])[0] 
        _nest2xyf ( _nside, _ipix, &_ix, &_iy, &_face_num) 
  ...
  with nogil:
    while size >0:
      while size > 0:
        _nside = (<long * > citer.data[0])[0] 
        _ipix = (<long * > citer.data[1])[0] 
        _ring2xyf ( _nside, _ipix, &_ix, &_iy, &_face_num) 
        (<int * > citer.data[2])[0] = _ix 
        (<int * > citer.data[3])[0] = _iy 
        (<int * > citer.data[4])[0] = _face_num 
        npyiter.advance(&citer)
  ...
  with nogil:
    while size >0:
      while size > 0:
        ...
        _ipix = _xyf2nest ( _nside, _ix, _iy, _face_num) 
        (<long * > citer.data[4])[0] = _ipix 
        npyiter.advance(&citer)
        size = size -1
      size = npyiter.next(&citer) 
  ...
  with nogil:
    while size >0:
      while size > 0:
        ...
        _iy = (<int * > citer.data[2])[0] 
        _face_num = (<int * > citer.data[3])[0] 
        _ipix = _xyf2ring ( _nside, _ix, _iy, _face_num) 
        (<long * > citer.data[4])[0] = _ipix 
        npyiter.advance(&citer)
        size = size -1
      size = npyiter.next(&citer) 
